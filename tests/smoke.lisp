  ...
  (%with-temporary-store-directory
   (lambda (directory)
     ...
     (let* ((dry-run-deleted (delete-registry-snapshot "snapshot-002" :directory directory :dry-run t))
            ...
            (dry-run-pruned (prune-registry-snapshots 1 :directory directory :dry-run t))
       ...
       (%check (and (vectorp dry-run-deleted-ids)
                    (= 1 (length dry-run-deleted-ids))
                    (string= "snapshot-001" (aref dry-run-deleted-ids 0)))
               "native store prune dry-run previews deleted snapshot ids")
       (%check (equal '("snapshot-003" "snapshot-001") after-dry-run-prune)
...
(defun %cli-usage-error-test ()
  (handler-case
    ...
    (cl-py:cli-usage-error ()
        ...
        (handler-case
                        (progn
                                (cl-py.internal:dispatch-top-level-command '("store" "delete-registry" "nightly"))
