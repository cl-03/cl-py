    ...
    ((= (length text) 5)
     (let* ((sign-char (char text 0))
            (sign (case sign-char
                    ...
                    (otherwise (%time-error "Invalid timestamp offset sign: ~A" sign-char))))
            (hours (%parse-fixed-integer text 1 3 "offset hour"))
            (minutes (%parse-fixed-integer text 3 5 "offset minute")))
       (unless (<= hours 23)
         (%time-error "Offset hour out of range: ~D" hours))
       (unless (<= minutes 59)
         (%time-error "Offset minute out of range: ~D" minutes))
...
(defun parse-iso-timestamp (text)
  (unless (member (length text) '(16 20 25) :test #'=)
    (%time-error "Unsupported ISO-8601 timestamp length: ~D" (length text)))
  (let* ((extended-format-p (and (>= (length text) 20)
                                 (char= (char text 4) #\-)
                                 (char= (char text 7) #\-)
                                 (char= (char text 10) #\T)
                                 (char= (char text 13) #\:)
                                 (char= (char text 16) #\:)))
         (basic-format-p (and (>= (length text) 16)
                              (char= (char text 8) #\T))))
    (unless (or extended-format-p basic-format-p)
      (%time-error "Timestamp must match YYYY-MM-DDTHH:MM:SSZ, YYYY-MM-DDTHH:MM:SS+HH:MM, YYYYMMDDTHHMMSSZ, or YYYYMMDDTHHMMSS+HHMM"))
    (let* ((year (%parse-fixed-integer text 0 4 "year"))
           (month (%parse-fixed-integer text (if extended-format-p 5 4) (if extended-format-p 7 6) "month"))
           (day (%parse-fixed-integer text (if extended-format-p 8 6) (if extended-format-p 10 8) "day"))
           (hour (%parse-fixed-integer text (if extended-format-p 11 9) (if extended-format-p 13 11) "hour"))
           (minute (%parse-fixed-integer text (if extended-format-p 14 11) (if extended-format-p 16 13) "minute"))
           (second (%parse-fixed-integer text (if extended-format-p 17 13) (if extended-format-p 19 15) "second"))
           (offset-minutes (%parse-iso-offset (subseq text (if extended-format-p 19 15)))))
