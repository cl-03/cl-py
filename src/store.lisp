...
(defun %inventory-entry< (left right)
  (string< (%inventory-entry-snapshot-id left)
           (%inventory-entry-snapshot-id right)))

(defun %inventory-summary-object (rows available-snapshot-count selector-matched-count)
  (if (null rows)
  (list :object
    (cons "available-snapshot-count" available-snapshot-count)
    (cons "selector-matched-count" selector-matched-count)
    (cons "adapter-count-filtered-out-count" selector-matched-count)
    ...
    (cons "smallest-snapshot-id" :null)
    (cons "smallest-snapshot" :null)
    (cons "total-adapter-count" 0)
    (cons "adapter-count-span" 0)
    (cons "min-adapter-count" :null)
    (cons "largest-snapshot-id" :null)
    (cons "largest-snapshot" :null)
  ...
  (let* ((oldest-entry (reduce (lambda (left right)
     ...
     (newest-entry (reduce (lambda (left right)
             (if (> (%inventory-entry-created-at-universal-time left)
                (%inventory-entry-created-at-universal-time right))
                 ...
                 right))
               rows))
               (smallest-entry (reduce (lambda (left right)
                       (let ((left-count (%inventory-entry-adapter-count left))
                       (right-count (%inventory-entry-adapter-count right)))
                   (if (or (< left-count right-count)
                     (and (= left-count right-count)
                          (%inventory-entry< left right)))
                       left
                       right)))
                     rows))
               (largest-entry (reduce (lambda (left right)
                      (let ((left-count (%inventory-entry-adapter-count left))
                      (right-count (%inventory-entry-adapter-count right)))
                  (if (or (> left-count right-count)
                    (and (= left-count right-count)
                      ...
                      right)))
                    rows))
                   (total-adapter-count (reduce #'+ rows :key #'%inventory-entry-adapter-count))
    ...
    (list :object
      (cons "available-snapshot-count" available-snapshot-count)
      (cons "selector-matched-count" selector-matched-count)
      (cons "adapter-count-filtered-out-count" (- selector-matched-count (length rows)))
      ...
      (cons "newest-created-at" (%inventory-entry-created-at newest-entry))
                (cons "smallest-snapshot-id" (%inventory-entry-snapshot-id smallest-entry))
                (cons "smallest-snapshot" smallest-entry)
      (cons "total-adapter-count" total-adapter-count)
      (cons "adapter-count-span" (- max-adapter-count min-adapter-count))
      (cons "min-adapter-count" min-adapter-count)
                (cons "largest-snapshot-id" (%inventory-entry-snapshot-id largest-entry))
                (cons "largest-snapshot" largest-entry)
...
(defun inventory-registry-snapshots (&key directory offset limit prefixes created-before created-after adapter-count-min adapter-count-max (sort "created-at-desc"))
  ...
  (when (and adapter-count-min adapter-count-max (> adapter-count-min adapter-count-max))
    (cl-py.internal:signal-cli-usage-error
     ...
     #'%print-store-usage))
  (let* ((available-snapshot-ids (list-registry-snapshots :directory directory))
         (selector-matched-snapshot-ids (%inventory-filtered-snapshot-ids directory prefixes created-before created-after))
         (matching-rows (remove-if-not (lambda (entry)
    ...
    (multiple-value-bind (paged-rows page)
      ...
      (list :object
            ...
            (cons "summary" (%inventory-summary-object matching-rows
                                                       (length available-snapshot-ids)
                                                       (length selector-matched-snapshot-ids)))
            (cons "sort" sort)
